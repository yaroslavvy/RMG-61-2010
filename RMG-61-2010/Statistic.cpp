...
int Statistic::selectCochranOrBartlettCriterion(FourDimArray * input, FourDimArray * dispersion, int component, int sampleName) {
	if (dispersion->getAmountOfSession(component, sampleName) < 2) {
		return 0;
	}//самое первое - проверка на необходимость отбора дисперсий по любым критери€м (отсутствие или 1 сесси€, т.е. отсутствие или 1 дисперси€ не могут работать ни с одним из критериев)
	...
	}// если среди сессий одного компонента и образца уже был критериальный отсев, то необходимо продолжить пользоватьс€ старым критерием вне зависимости от всех остальных факторов
	if ((dispersion->getAmountOfSession(component, sampleName) > 40)) {
		return 2;
	}// если более 40 сессий, то работает только Ѕартлетт
	int i = 0;
	...
	for (int s = 0; s < input->getAmountOfSession(); s++) {
		if (input->getAmountOfParallel(s, component, sampleName) > 1) {
			if (input->getAmountOfParallel(s, component, sampleName) > 6) {
				return 2;
			}// если в какой-то сессии более 6 параллельных испытаний, то работает только Ѕартлетт
	...
	if (Statistic::arrayElementsAreEqual(arrayOfParallels, input->getAmountOfSession(component, sampleName))) {
		return 1;// количество параллелей во всех сесси€х одинаково, значит  охрен
	}
	else {
		return 2;// количество параллелей в сесси€х неодинаково, значит Ѕартлетт
	}
}
...
bool Statistic::cochranCriterionCalculate(FourDimArray * input, FourDimArray * dispersion, int component, int sampleName) {
	...
	if ((maxDispersion / totalSumOfDispersions) > Statistic::cochranCriticalValues((dispersion->getAmountOfSession(component, sampleName)), amountOfParallels)) {
		for (int p = 0; p < input->getAmountOfParallel(/*maxDispersionSession, component, sampleName*/); p++) {// кажетс€ перегрузка функции не работает как надо
